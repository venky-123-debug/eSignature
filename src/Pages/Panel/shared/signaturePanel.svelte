...
<div class:hidden={!showSignPanel && !showPdf} class="relative z-10">
  ...
  <div class="fixed inset-0 z-40 flex w-screen flex-col items-center justify-center gap-3 overflow-y-auto">
    ...
    <canvas on:mousedown on:mousemove on:mouseup on:mouseout on:touchstart on:touchmove on:touchend bind:this={canvas} width="400" height="200" class="z-30 cursor-pointer rounded-md border border-gray-500 bg-white" />
    <div class="flex items-center justify-center gap-3">
      ...
      <button on:click={() => dispatch("ok")} type="button" class="flex min-w-fit items-center justify-center rounded-md bg-green-500 px-3 py-1.5 text-sm font-semibold text-white hover:bg-green-600 active:bg-green-500">Place Signature</button>
