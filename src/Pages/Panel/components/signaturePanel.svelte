...
<div class:hidden={!showSignPanel && !showPdf} class="relative z-10">
  ...
  <div class="fixed inset-0 z-40 flex w-screen flex-col items-center justify-center gap-3 overflow-y-auto">
    ...
    <div class="relative">
      <canvas on:mousedown on:mousemove on:mouseup on:mouseout on:touchstart on:touchmove on:touchend bind:this={canvas} width="400" height="200" class="cursor-pointer rounded-md border border-gray-500 bg-white" />
      <div class="bottom-1.5 absolute inset-x-0 flex w-full items-center justify-between px-1.5">
